...
# 幾何配對雷達 (與之前完全相同)
# ==========================================
def build_periodic_geometric_maps(x_nodes, y_nodes, fmask, Lx=1.0, Ly=1.0):
    """具備空間穿越能力的週期性雷達"""
    K, Np = x_nodes.shape
    Nfaces = 3
    Nfp = len(fmask[0])
    ...
    for k1 in range(K):
        for f1 in range(Nfaces):
            ...
            if neighbors:
                ...
                if abs(cx - 0.0) < TOL: target_cx = Lx
                elif abs(cx - Lx) < TOL: target_cx = 0.0
                if abs(cy - 0.0) < TOL: target_cy = Ly
                elif abs(cy - Ly) < TOL: target_cy = 0.0
                
                partner_coords = (round(target_cx, 8), round(target_cy, 8))
                if partner_coords in center_to_face:
                    k2, f2 = center_to_face[partner_coords][0]
                    nodes1, nodes2 = vmapM[k1, f1, :], vmapM[k2, f2, :]
                    # 補償平移量，進行點對點鎖定
                    shift_x, shift_y = cx - target_cx, cy - target_cy
                    for i, n1 in enumerate(nodes1):
                        dist = (x_flat[nodes2] + shift_x - x_flat[n1])**2 + (y_flat[nodes2] + shift_y - y_flat[n1])**2
                        vmapP[k1, f1, i] = nodes2[np.argmin(dist)]
                else:
...
def q_exact(x, y, t, cx=1.0):
    # 加入 2*np.pi，確保頭尾相連
    return np.sin(2.0 * np.pi * (x - cx * t))
...
def run_lsrk_pure_time_test():
    ...
    vmapM, vmapP = build_periodic_geometric_maps(x_nodes, y_nodes, engine.fmask)
